...
static int OEN(int entry, void const *param)
{
    ...
    switch (entry)
    {
        ...
        case 2:
        {
            ...
            memcpy(assembler_card._BUFCARD.COMM, "Выход из программы", 18);
            ...
            for (i = 0; i < ISYM; i++)
            {
                if (isalpha(SYM[i].NAME[0]))
                {
                    if ('B' == SYM[i].TYPE)
                    {
                        ...
                        memcpy(assembler_card._BUFCARD.COMM, "Определение переменной", 22);
                        ZKARD ();
                    }
                }
            }
            ...
            memcpy(assembler_card._BUFCARD.COMM, "Конец программы", 15);
                        ...
                        && strlen(SYM[i].NAME) == strlen(FORMT[0]))
                {
                    if (SYM[i].TYPE == 'B')
                    {
                        ...
                        else
                        {
                            ...
                            memcpy(assembler_card._BUFCARD.COMM, "Формирование значения арифм.выражения", 37);
                            ZKARD();
...
static int OPR(int entry, void const *param)
{
    ...
    switch (entry)
    {
        ...
        case 2:
        {
            ...
            memcpy(assembler_card._BUFCARD.COMM, "Начало программы", 16 );
            ...
            memcpy(assembler_card._BUFCARD.COMM, "Загрузить регистр базы", 22);
            ...
            memcpy(assembler_card._BUFCARD.COMM, "Назначить регистр базой", 23);
...
static int PRO(int entry, void const *param)
{
    switch(entry)
    {
        ...
        case 2:
        {
            ...
            p_asm_f = fopen(p_asm_fp_name, "wb");
            if (NULL == p_asm_f)
            {
                return 7;
            }
            ...
            {
                fwrite(ASSTXT, 80, IASSTXT, p_asm_f);
            }
